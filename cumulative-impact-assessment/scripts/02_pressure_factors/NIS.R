...
NIS_250 <- terra::rast(file.path(PATHS$input_pressure, "NIS","_ags_PL_17","PL_17.tif")) %>%
...
terra::writeRaster(
  NIS_250,
  filename = file.path(PATHS$output_ecosystem_tif,"fisk", "/NIS_250.tif"),
  overwrite = TRUE
)
...
ggsave(plot = map_NIS,
       filename = file.path(PATHS$output_pressure_png,"/NIS/" ,"NIS.png"),
       bg = NULL,
       height = 18,
       width = 18,
       dpi = 300)
